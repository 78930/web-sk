...
export default function Navbar() {
  ...
  // Add drop-shadow when user scrolls down
  useEffect(() => {
...
<<<<<<< HEAD
    `text-sm font-medium transition-colors ${isActive ? "text-brand-600 dark:text-brand-400" : "text-slate-600 hover:text-brand-600 dark:text-slate-300 dark:hover:text-brand-400"}`;

  return (
    <>
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:left-2 focus:top-2 focus:z-[100] focus:rounded-lg focus:bg-brand-600 focus:px-4 focus:py-2 focus:text-sm focus:font-semibold focus:text-white focus:shadow-lg">
...
>>>>>>> 105e7d2 (web)
          ...
          <div className="hidden items-center gap-3 lg:flex">
            <button onClick={toggleTheme} aria-label="Toggle dark mode" aria-pressed={theme === "dark"} className="btn-ghost h-10 w-10 !px-0">
              {theme === "dark" ? <Icon.Sun className="h-5 w-5" /> : <Icon.Moon className="h-5 w-5" />}
            </button>
            {isAuthenticated ? (
              <>
<<<<<<< HEAD
                <Link to="/dashboard" className="btn-secondary">{user?.name?.split(" ")[0] || "Dashboard"}</Link>
                <button onClick={handleLogout} className="btn-ghost" aria-label="Log out"><Icon.Logout className="h-5 w-5" /></button>
              </>
            ) : (
              <>
                <Link to="/login" className="btn-ghost">Log in</Link>
          ...
          <div className="flex items-center gap-1 lg:hidden">
            ...
            <button onClick={() => setOpen((v) => !v)} aria-label="Toggle menu" aria-expanded={open} className="btn-ghost h-10 w-10 !px-0">
            ...
            <div className="flex flex-col gap-1">
              {NAV.map((item) => (
                <NavLink key={item.to} to={item.to} end={item.end} onClick={() => setOpen(false)}
                  className={({ isActive }) => `rounded-lg px-3 py-2.5 text-sm font-medium ${isActive ? "bg-brand-50 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300" : "text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"}`}
                >
                  {item.label}
                </NavLink>
              ))}
              <div className="mt-3 flex flex-col gap-2 border-t border-slate-200 pt-3 dark:border-slate-700">
                {isAuthenticated ? (
                  <>
                    <Link to="/dashboard" onClick={() => setOpen(false)} className="btn-secondary w-full text-center">{user?.name?.split(" ")[0] || "Dashboard"}</Link>
                    <button onClick={handleLogout} className="btn-ghost w-full">Log out</button>
                  </>
                ) : (
                  <>
                    <Link to="/login" onClick={() => setOpen(false)} className="btn-ghost w-full text-center">Log in</Link>
